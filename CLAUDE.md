...
pnpm db:migrate          # apply migrations (drizzle/migrations)
pnpm db:seed             # seed from IMDb TSVs in seeds/ (see README for required files)
```
...
**Request pipeline (`src/app.ts`):** CORS → 1 MB body-size guard → request logger → routes → OpenAPI docs → `onError` (returns the unified error envelope). `GET /health` pings the DB.
...
**Auth:** `src/lib/jwt.ts` (HS256, 24h TTL via `hono/jwt`) + bcrypt (`src/lib/password.ts`). `src/middleware/auth.ts` `requireAuth` rejects with 401; `optionalAuth` populates `c.get('user')` as `user | null` (used by movie detail for `isFavorite`). Login runs a dummy bcrypt compare on unknown emails to equalize timing.
...
- **Error envelope:** never hand-roll error JSON. Use `errorResponse(c, ErrorCode.X, message)` from `src/lib/errors.ts`. Every `OpenAPIHono` is constructed with `{ defaultHook }` so Zod validation failures return the same `{ error: { code, message, details? } }` shape (400 `VALIDATION_ERROR`).
...
- **DB connection string:** `src/env.ts` validates env with Zod and accepts `POSTGRES_URL` as a fallback for `DATABASE_URL` (Nile/Vercel expose the former). `JWT_SECRET` must be ≥32 chars.
