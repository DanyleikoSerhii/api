...
--
-- Instead we converge with guarded DDL that needs only ALTER/CREATE on existing
-- objects. It is safe to run repeatedly and mirrors migrations 0001–0003.
-- Keep this in sync when new additive migrations land. Base tables (0000) are
-- assumed to already exist on prod.
...
-- 0003: trigram index for title search (best-effort; needs the pg_trgm
-- extension, which may be unavailable on managed Postgres).
DO $$
BEGIN
	BEGIN
